...
repeated "Change WiFi"/"Change MQTT" invocations: (1) an unpatched LVGL bug
...
cleanly, and (2) a local `lv_conf.h` resolution bug
([lvgl/lvgl#10356](https://github.com/lvgl/lvgl/issues/10356)) that left
LVGL silently running on a 64KB internal-SRAM heap instead of the intended
512KB SDRAM-backed pool, with no error. **If LVGL is ever reinstalled or
updated, the patches in `lvgl_patches/` must be reapplied** (or
re-verified as already fixed upstream) before trusting this fix. A
`logMemStatus()` helper (calls `lv_mem_monitor()`) is left in place after
every Change WiFi/Change MQTT flow as an ongoing regression signal — watch
`free_size`/`frag_pct` for a downward/upward trend if hangs ever recur.
