...
void lv_obj_set_parent(lv_obj_t * obj, lv_obj_t * parent)
{
    ...
    //Upstream had this pair of lv_realloc() calls unchecked (LVGL issue
    //#9794, fixed in #9795 -- landed after our v9.5.0), the second one
    //(growing the new parent's array) immediately indexed into a possibly-
    //NULL result. Backported minimally: grow the new parent's array FIRST
    //and bail out before touching the old parent if that fails, so obj is
    //left validly attached to its original parent rather than corrupted or
    //silently detached from both.
    lv_obj_t ** new_children = lv_realloc(parent->spec_attr->children,
                                          (parent->spec_attr->child_cnt + 1) * (sizeof(lv_obj_t *)));
    if(new_children == NULL) {
        LV_LOG_ERROR("Failed to reparent object, out of memory");
        return;
    }
    ...
    if(old_parent->spec_attr->child_cnt) {
        //A failed shrink leaves the original (slightly oversized) block
        //intact and usable -- only adopt the result if it actually succeeded.
        lv_obj_t ** shrunk = lv_realloc(old_parent->spec_attr->children,
                                        old_parent->spec_attr->child_cnt * (sizeof(lv_obj_t *)));
        if(shrunk != NULL) {
            old_parent->spec_attr->children = shrunk;
        }
    }
    else {
...
static void obj_delete_core(lv_obj_t * obj)
{
    ...
    if(obj->parent == NULL) {
        ...
        disp->screen_cnt--;
        //Same shrink-only realloc pattern as below -- a failed shrink leaves
        //the original block intact and usable, so only adopt the result if
        //it succeeded (see comment in lv_obj_set_parent above).
        lv_obj_t ** shrunk_screens = lv_realloc(disp->screens, disp->screen_cnt * sizeof(lv_obj_t *));
        if(shrunk_screens != NULL || disp->screen_cnt == 0) {
            disp->screens = shrunk_screens;
        }
    }
    /*Remove the object from the child list of its parent*/
        ...
        lv_obj_t ** shrunk_children = lv_realloc(obj->parent->spec_attr->children,
                                                 obj->parent->spec_attr->child_cnt * sizeof(lv_obj_t *));
        if(shrunk_children != NULL || obj->parent->spec_attr->child_cnt == 0) {
            obj->parent->spec_attr->children = shrunk_children;
        }
    }
