...
lv_obj_t * lv_obj_class_create_obj(const lv_obj_class_t * class_p, lv_obj_t * parent)
{
    ...
    /*Create a normal object*/
        ...
        //Upstream lv_realloc() here was unchecked (LVGL issue #9794, fixed
        //in #9795 -- landed after our v9.5.0). A NULL return (heap
        //exhausted/fragmented) was immediately indexed into, corrupting
        //memory instead of failing cleanly. Backported minimally here.
        lv_obj_t ** new_children = lv_realloc(parent->spec_attr->children,
                                              sizeof(lv_obj_t *) * (parent->spec_attr->child_cnt + 1));
        LV_ASSERT_MALLOC(new_children);
        if(new_children == NULL) {
            LV_LOG_ERROR("Failed to grow parent's children array, out of memory");
            lv_free(obj);
            return NULL;
        }
