...
#define LV_STDARG_INCLUDE       <stdarg.h>
     ...
     *  Backed by the Giga's external 8MB SDRAM via ea_malloc(), not
     *  internal SRAM -- see LV_MEM_POOL_ALLOC below. A real hang
     *  building the Unit 2 dashboard's WiFi scan list (confirmed via
     *  bisection + real-hardware trace logging, 2026-07-17) was
     *  ultimately traced to the mbed RTOS main thread's STACK, not this
     *  heap pool (see that sketch's mbed_app.json) -- but this move
     *  still stands on its own merits: the Giga has 8MB of essentially
     *  idle SDRAM sitting unused by default, and moving LVGL's pool off
     *  the small, scarce internal SRAM (shared with the WiFi
     *  module/RTOS/etc.) gives generous headroom for any LVGL project's
     *  widget growth, at negligible cost against the 8MB available. */
    ...
    #define LV_MEM_ADR 0     /**< 0: unused -- using LV_MEM_POOL_ALLOC below instead of a fixed address */
    /* Instead of an address give a memory allocator that will be called to get a memory pool for LVGL. E.g. my_malloc */
    #define LV_MEM_POOL_INCLUDE "ea_malloc.h"
...
//Was &lv_font_montserrat_14 (the stock template's default), but only
//LV_FONT_MONTSERRAT_32 is enabled above -- harmless while lv_conf.h's
//own values weren't reliably reaching every translation unit (see
//LV_CONF_PATH's comment in lv_conf_internal.h), now a real build error
//once they do.
